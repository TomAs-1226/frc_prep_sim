...
struct PreMatchView: View {
    ...
    private var buildStep: some View {
        VStack(spacing: 16) {
            ...
            // Accent Color
            VStack(alignment: .leading, spacing: 8) {
                sectionLabel("Accent Color", icon: "paintpalette.fill")
                HStack(spacing: 10) {
                    ForEach(RobotBuild.AccentColor.allCases) { accent in
                        Button(action: {
                            withAnimation(.easeInOut(duration: 0.2)) { selectedBuild.accentColor = accent }
                        }) {
                            Circle()
                                ...
                                .overlay(
                                    Circle()
                                        .strokeBorder(selectedBuild.accentColor == accent ? .white : .clear, lineWidth: 2)
                                )
                        }
                    }
                    Spacer()
            ...
            tipBanner(text: "Every build has tradeoffs! Swerve+Elevator scores high but is fragile. Tank+Simple cycles fast and gets deep climb (12pts). Tune your robot for your playstyle!")
        }
    }
    ...
    private var tuningSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            tuningSlider(
                label: "Gear Ratio",
                value: $selectedBuild.tuning.gearRatio,
                leftLabel: "Torque",
                rightLabel: "Speed",
            ...
            tuningSlider(
                ...
                value: $selectedBuild.tuning.weightBalance,
                leftLabel: "Traction",
                rightLabel: "Pushing",
                leftIcon: "figure.walk",
                rightIcon: "arrow.forward.square.fill"
            )

            tuningSlider(
                label: "Mechanism Tuning",
                value: $selectedBuild.tuning.mechanismTuning,
                leftLabel: "Fast",
                rightLabel: "Precise",
                leftIcon: "bolt.fill",
                rightIcon: "scope"
    ...
    private func tuningSlider(label: String, value: Binding<Double>,
                                ...
                                leftIcon: String, rightIcon: String) -> some View {
        VStack(spacing: 4) {
            HStack {
                Text(label)
                    .font(.system(size: 10, weight: .bold))
